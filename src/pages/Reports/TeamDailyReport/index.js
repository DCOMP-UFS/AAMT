...
const TeamDailyReport = () => {
  ...
  const { equipe_id, data } = route.params;

  const auxData = data.split("-")
  const dataString = auxData[2]+"/"+auxData[1]+"/"+auxData[0]
  ...
  useEffect(() => {
    async function loadActivity() {
      try {
        ...
        let url2 = `/relatorios/equipe/${equipe_id}/data/${data}`;
    ...
    <>
      {loading ? (
        ...
        <Container>
          <Card>
            <Header>
              <PropertyTitle>Dia - {dataString}</PropertyTitle>
            </Header>
          </Card>
          <Card>
            <Header>
